...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt,
                                         ...
                                         MIT_UserParameters *parameters) : iterationsBetweenMPC(_iterations_between_mpc),
                                                                           ...
                                                                           //  pacing(horizonLength, Vec4<int>(0, 5, 0, 5), Vec4<int>(5, 5, 5, 5), "Pacing"),
                                                                           ...
                                                                           CoM_init(12, 8)
{
  ...
  csvLog.open("convexLocomotionData.txt");
...
void ConvexMPCLocomotion::computex_des(ControlFSMData<float> &data)
{
  ...
  auto &cheaterState = data._stateEstimator->getCheaterState();
  Vec3<float> vWorld = (ori::quaternionToRotationMatrix(cheaterState.orientation).transpose() * cheaterState.vBody).cast<float>();
  // Vec3<float> vWorld = seResult.vWorld;
  // Vec3<float> vWorld = data._stateEstimator->getMyLinearVel();
  ...
  sta << seResult.position[0], vWorld[0], seResult.position[1], vWorld[1];
...
void ConvexMPCLocomotion::CoMinit(ControlFSMData<float> &data, float t)
{
  ...
  Vec8<float> beq;
  beq << seResult.position(0), seResult.position(1), x_qp(0), x_qp(1), vWorld(0), vWorld(1), 0, 0;
...
void ConvexMPCLocomotion::trajOpt(ControlFSMData<float> &data)
{
  ...
  x_predict.resize(4 * horizon, 1);
  x_predict.setZero();
  x_init.block(0, 0, 2, 1) = Vec2<float>(seResult.position(0), vWorld(0));
  x_init.block(2, 0, 2, 1) = Vec2<float>(seResult.position(1), vWorld(1));
...
void ConvexMPCLocomotion::changeSupportIfNeeded(ControlFSMData<float> &data)
{
  ...
  stat[2] = 0;
  // stat[3] = seResult.vWorld[1]; // imu
  stat[3] = vWorld[1];

  Vec8<float> A_pls;
  A_pls = A_pl * stat;
  v_com1 = Vec2<float>(vWorld(0), vWorld(1));
...
void ConvexMPCLocomotion::run(ControlFSMData<float> &data)
{
  ...
  /* different with MIT --- start ---------------------*/
  Timer SQPTimer;
  if (iterationCounter % 14 == 0 && !changesupport)
  {
...
void ConvexMPCLocomotion::solveSparseMPC(int *mpcTable, ControlFSMData<float> &data)
{
  ...
  _sparseCMPC.setX0(seResult.position, vWorld, seResult.orientation, seResult.omegaWorld);

...
void Simulation::highLevelControl()
{
  ...
  if (!start_test)
  {
    _sharedMemory().simToRobot.cheaterState.acceleration = 1.0 * Vec3<double>(vx /  _simParams.high_level_dt, vy /  _simParams.high_level_dt, 0);
    _sharedMemory().simToRobot.vectorNav.accelerometer = 1.0 * Vec3<float>(vx /  _simParams.high_level_dt, vy /  _simParams.high_level_dt, 0);
    vx = 0;
    vy = 0;
  }
  _sharedMemory().simulatorIsDone();

  ...
  random2(horizonLength, Vec4<int>(8,16,16,8), 0.5, "Double Trot")
{
  ...
  prob_ = std::make_unique<convexmpc::Prob>();
  convexmpc::ProbConfig prob_config;
  prob_config.dt = dtMPC;
  prob_config.horizon = horizonLength;
  prob_config.mu = 0.4;
  prob_config.f_max = 120;
  prob_config.alpha = 4e-5;
  prob_->updateProbConfig(prob_config);
...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data) {
  ...
  prob_->updateControlFSMData(data);
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
  ...
  prob_->updateXDrag(x_comp_integral);
  ...
  assert(!prob_->solve(solution));

  for(int leg = 0; leg < 4; leg++)
  {
    Vec3<float> f;
    for(int axis = 0; axis < 3; axis++)
      f[axis] = solution[leg*3 + axis];

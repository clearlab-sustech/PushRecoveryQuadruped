										 ...
										 const real_t *const lbA_new, const real_t *const ubA_new)
{
	...
	if ((lbA_new != 0) && (ubA_new != 0))
	{
		for (i = 0; i < nC; ++i)
		{
			...
			if ((lbA_new[i] < -INFTY + options.boundTolerance) && (ubA_new[i] > INFTY - options.boundTolerance) && (options.enableFarBounds == BT_FALSE))
			{
				constraints.setType(i, ST_UNBOUNDED);
			}
			else
			{
				if (options.enableEqualities && lbA[i] > ubA[i] - options.boundTolerance && lbA_new[i] > ubA_new[i] - options.boundTolerance)
...
returnValue QProblem::obtainAuxiliaryWorkingSet(const real_t *const xOpt, const real_t *const yOpt,
												const Bounds *const guessedBounds, const Constraints *const guessedConstraints,
												Bounds *auxiliaryBounds, Constraints *auxiliaryConstraints) const
{
	int_t i = 0;
	int_t nV = getNV();
	int_t nC = getNC();
...
returnValue QProblem::updateFarBounds(real_t curFarBound, int_t nRamp,
									  ...
									  const real_t *const ubA_new, real_t *const ubA_new_far) const
{
	int_t i;
	real_t rampVal, t;
	int_t nV = getNV();

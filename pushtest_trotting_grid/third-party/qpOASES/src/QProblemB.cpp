...
returnValue QProblemB::setupSubjectToType(const real_t *const lb_new, const real_t *const ub_new)
{
	...
	if ((lb_new != 0) && (ub_new != 0))
	{
		for (i = 0; i < nV; ++i)
		{
			if ((lb_new[i] < -INFTY + options.boundTolerance) && (ub_new[i] > INFTY - options.boundTolerance) && (options.enableFarBounds == BT_FALSE))
			{
				bounds.setType(i, ST_UNBOUNDED);
			}
			else
			{
				if (options.enableEqualities && lb[i] > ub[i] - options.boundTolerance && lb_new[i] > ub_new[i] - options.boundTolerance)
...
returnValue QProblemB::loadQPvectorsFromFile(const char *const g_file, const char *const lb_file, const char *const ub_file,
											 real_t *const g_new, real_t *const lb_new, real_t *const ub_new) const
{
	int_t nV = getNV();
...
returnValue QProblemB::updateFarBounds(real_t curFarBound, int_t nRamp,
									   ...
									   const real_t *const ub_new, real_t *const ub_new_far) const
{
	int_t i;
	real_t rampVal, t;
	int_t nV = getNV();

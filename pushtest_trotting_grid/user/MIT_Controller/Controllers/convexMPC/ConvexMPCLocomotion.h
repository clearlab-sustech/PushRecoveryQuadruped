...
private:
  ...
  void changeSupportIfNeeded(ControlFSMData<float> &data);
  ...
  void initSparseMPC();
  void computex_des(ControlFSMData<float> &data);
  ...
  DMat<float> Aa, Ba, Sx, Su, f_opt_traj, H_traj, f_traj, Q_w, R_w, x_predict, x_next;
  ...
  ofstream csvLog;
  DVec<float> Uopt_vec, x_next_vec, horizon_vec;
};

...
void WBC_Ctrl<T>::_UpdateLegCMD(ControlFSMData<T> &data)
{
  ...
  {
    // abad
    T abad_l = (leg % 2 == 0 ? -1.57 : -1.2);
    if (cmd[leg].qDes[0] < abad_l)
    {
      cmd[leg].qDes[0] = abad_l;
    }
    if (data._legController->datas[leg].q[0] < abad_l)
    {
      T abad_pos = data._legController->datas[leg].q[0] - abad_l;
      cmd[leg].tauFeedForward[0] = 1. / (abad_pos * abad_pos + 0.02);
      cmd[leg].kpJoint.setZero();
      cmd[leg].kdJoint.setZero();
    }

    // hip
    if (cmd[leg].qDes[1] < -3.14)
    {
      cmd[leg].qDes[1] = -3.14;
    }
    if (data._legController->datas[leg].q[1] < -3.14)
    {
      T hip_pos = data._legController->datas[leg].q[1] + 3.14;
      cmd[leg].tauFeedForward[1] = 1. / (hip_pos * hip_pos + 0.02);
      cmd[leg].kpJoint.setZero();
      cmd[leg].kdJoint.setZero();
    }

    // knee
    if (cmd[leg].qDes[2] < 0.1)
    {
      cmd[leg].qDes[2] = 0.1;
    }
    if (data._legController->datas[leg].q[2] < 0.1)
    {
  ...
  for (size_t leg(0); leg < 4; ++leg)
  {
    ...
    // hip
    if (cmd[leg].qDes[1] > 1.57)
    {
      cmd[leg].qDes[1] = 1.57;
    }
    if (data._legController->datas[leg].q[1] > 1.57)
    {
      T hip_pos = data._legController->datas[leg].q[1] - 1.57;
      cmd[leg].tauFeedForward[1] = -1. / (hip_pos * hip_pos + 0.02);
      cmd[leg].kpJoint.setZero();
      cmd[leg].kdJoint.setZero();
    }
    ...
    // knee
    if (cmd[leg].qDes[2] > 3.0)
    {
      cmd[leg].qDes[2] = 3.0;
    }
    if (data._legController->datas[leg].q[2] > 3.0)
    {
      T knee_pos = data._legController->datas[leg].q[2] - 3.0;
      cmd[leg].tauFeedForward[2] = -1. / (knee_pos * knee_pos + 0.02);
      cmd[leg].kpJoint.setZero();
      cmd[leg].kdJoint.setZero();
    }

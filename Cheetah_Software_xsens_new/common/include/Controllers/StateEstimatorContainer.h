...
public:
                          ...
                          RobotControlParameters *parameters)
  {
    ...
    if (!csvLog)
    {
      throw std::runtime_error("can not open simVSreal.txt");
    }
    triggle = false;
  }
  ...
  void run(CheetahVisualization *visualization = nullptr)
  {
    ...
    // csvoutN(csvLog, _data.result->omegaWorld, 3, false);
    // csvoutN(csvLog, my_linearVel, 3, false);
    // csvoutN(csvLog, _data.vectorNavData->accelerometer, 3, false);
    // csvLog << std::endl;
  }
  ...
  void my_estimate()
  {
    ...
    if (triggle != old_triggle)
    {
      my_linearVel.setZero();
    }
    for (int i = 0; i < 3; i++)
    {
      my_linearVel[i] = my_linearVel[i] + (acc[i] > 80 ? 0.002 * acc[i] : 0);
    }
  }

...
void MiniCheetahHardwareBridge::run() {
  ...
  // Xsens
  if(_XsensInit)
    _XsensThread = std::thread(&MiniCheetahHardwareBridge::runXsens, this);
...
void MiniCheetahHardwareBridge::runXsens()
{
  while (true)
  {
    // Timer t;
    // printf("run xsens\n");
    imuxsens.imu_update();
    // printf("imu update time: %.4f\n", t.getMs());
#ifdef USE_XSENS
    // _vectorNavData = imuxsens.imudata;
    _vectorNavData.accelerometer = imuxsens.imudata.accelerometer;
    _vectorNavData.quat[0] = imuxsens.imudata.quat[1];
    _vectorNavData.quat[1] = imuxsens.imudata.quat[2];
    _vectorNavData.quat[2] = imuxsens.imudata.quat[3];
    _vectorNavData.quat[3] = imuxsens.imudata.quat[0];
    _vectorNavData.gyro = imuxsens.imudata.gyro;
    // printf("imu loop time: %.4f\n", t.getMs());
    usleep(500);
#endif
  }
}
...
#ifdef USE_XSENS
  printf("Init Xsens...\n");
  if (!imuxsens.imu_init())
  {
    printf("Xsens failed to initialize\n");
  }
  else
  {
    printf("Succeed!\n");

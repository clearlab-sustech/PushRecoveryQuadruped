...
//    std::thread _PointCloudThread = std::thread(&FSM_State_Vision::runPointCloud, this);
//    _PointCloudThread.join();


    printf("[FSM VISION] On Enter\n");
}
template <typename T>
void FSM_State_Vision<T>::runPointCloud()
{
...
void FSM_State_Vision<T>::onExit() {
    ...
    iter = 0;
    int tm =system("gnome-terminal -t \"title-name\" -x bash -c \"sh /home/user/Cheetah_software/kill_pointcloud.sh;exec bash;\" ");
    std::cout<<"point cloud killed "<<tm<<std::endl;
}

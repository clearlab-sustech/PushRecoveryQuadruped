...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt,
                                         ...
                                         MIT_UserParameters *parameters) : iterationsBetweenMPC(_iterations_between_mpc),
                                                                           ...
                                                                           //  trotting(horizonLength, Vec4<int>(5, 0, 0, 5), Vec4<int>(5, 5, 5, 5), "Trotting"),
                                                                           ...
                                                                           CoM_init(12, 8)
{
  ...
  csvLog.open("./expdata.txt");
  foot_hold_log.open("./foothold.txt");
}

bool safe = true;
bool changesupport = true;
bool transition = true;
void ConvexMPCLocomotion::computex_des_bounding(ControlFSMData<float> &data)
{
  ...
  // b_pl <<0.067743521,10,10,0.10840929,0.10840929,-0.057887955,10,10;
  Vec8<float> C_1;
  ...
  // matrix_to_real_t(A_qp, C, 14, 2);
  // matrix_to_real_t(Aub_qp, ub, 14, 1);
  ...
  if (rval != qpOASES::SUCCESSFUL_RETURN)
    printf("computex_des_bounding: qp failed to initialize\n");

  int rval2 = CoMOpt.getPrimalSolution(sln);
  if (rval2 != qpOASES::SUCCESSFUL_RETURN)
    printf("computex_des_bounding: qp failed to solve!\n");
...
void ConvexMPCLocomotion::computex_des_trotting(ControlFSMData<float> &data)
{
  auto seResult = data._stateEstimator->getResult();
  Vec2<float> vd(0.0f, 0.0f);
  Eigen::Matrix<float, 2, 2> H;
  ...
  int rval = CoMOpt.init(H_qp, g_qp, A_qp, NULL, NULL, NULL, Aub_qp, nWSR);
  if (rval != qpOASES::SUCCESSFUL_RETURN)
    printf("computex_des_trotting: qp failed to initialize\n");

  int rval2 = CoMOpt.getPrimalSolution(sln);
  if (rval2 != qpOASES::SUCCESSFUL_RETURN)
    printf("computex_des_trotting: qp failed to solve!\n");
...
void ConvexMPCLocomotion::CoMinit(ControlFSMData<float> &data, float t)
{
  ...
  if (rval != qpOASES::SUCCESSFUL_RETURN)
  {
    printf("CoMinit: qp failed to initialize\n");
    optv = 1e6;
  }
  else
  {
  ...
  if (rval2 != qpOASES::SUCCESSFUL_RETURN)
    printf("CoMinit: qp failed to solve!\n");
...
void ConvexMPCLocomotion::trajOpt(ControlFSMData<float> &data)
{
  ...
  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Ca, du, dl;
  ...
  contact_table.resize(horizonLength * 4 * 80, 1);
  ...
  for (int ith_horizon = 0; ith_horizon < horizon; ith_horizon++)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      if (contact_table(ith_horizon * 4 + i) == 1)
      {
        ...
        csvoutN(foot_hold_log, pFoot_u[j], 2, firstRun);
        j++;
      }
    }
    ...
    {
      Q_w.block(ith_horizon * 4, ith_horizon * 4, 4, 4) = 1000000 * Q_i;
    }
  ...
  // std::cout << "Q_w: \n" << Q_w << std::endl;
  // std::cout << "R_w: \n" << R_w << std::endl;
  /* Ca.bottomRows(4) = Su.bottomRows(4);
  Vec4<float> slack_v;
  slack_v << 0.1, 1.0, 0.1, 1.0;
  du.bottomRows(4) = x_predict.bottomRows(4) - Sx.bottomRows(4) * x_init + slack_v;
  dl.bottomRows(4) = x_predict.bottomRows(4) - Sx.bottomRows(4) * x_init - slack_v; */

  // Compute H and f
  H_traj = Su.transpose() * Q_w * Su + R_w;
  f_traj = Su.transpose() * Q_w * Sx * x_init - Su.transpose() * Q_w * x_predict;
  // std::cout << "H_traj: \n" << H_traj << std::endl;
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> H_traj_d, f_traj_d, Ca_d, dl_d, du_d;
  H_traj_d = H_traj.cast<double>();
  f_traj_d = f_traj.cast<double>();
  Ca_d = Ca.cast<double>();
  dl_d = dl.cast<double>();
  du_d = du.cast<double>();

  nWSR = 1000;
  Timer timer_trajOpt_solve;
  if (qpOASES::SUCCESSFUL_RETURN != trajSolver->init(H_traj_d.data(), f_traj_d.data(), Ca_d.data(), NULL, NULL, dl_d.data(), du_d.data(), nWSR))
  {
    // throw std::runtime_error("trajSolver qp failed");
    x_next.resize(horizon * 4, 1);
    for (int i = 0; i < horizon; i++)
    {
      float ratio = pow(float(i + 1) / float(horizon), 0.2);
      x_next(i * 4, 0) = seResult.position(0) + ratio * (x_qp(0) - seResult.position(0));
      x_next(i * 4 + 1, 0) = (1.0 - ratio) * seResult.vWorld(0);
      x_next(i * 4 + 2, 0) = seResult.position(1) + ratio * (x_qp(1) - seResult.position(1));
      x_next(i * 4 + 3, 0) = (1.0 - ratio) * seResult.vWorld(1);
    }
  }
  else
  {
    DVec<double> f_opt_traj_d;
    f_opt_traj_d.resize(H_traj.rows());
    trajSolver->getPrimalSolution(f_opt_traj_d.data());
    f_opt_traj = f_opt_traj_d.cast<float>();
    v_opt_traj = trajSolver->getObjVal();
    printf("Time TrajOpt solve: %f\n", timer_trajOpt_solve.getMs());
    x_next.resize(horizon * 4, 1);
    x_next = Sx * x_init + Su * f_opt_traj;
  }
  DVec<float> x_predict_v = x_predict;
  DVec<float> x_next_v = x_next;
  csvoutN(csvLog, x_next_v, horizon * 4, firstRun);
  csvLog << "\n";
  csvoutN(csvLog, x_predict_v, horizon * 4, firstRun);
  csvLog << "\n";
}
...
bool foot1c = true;
bool foot2c = true;
void ConvexMPCLocomotion::changeSupportIfNeeded_bounding(ControlFSMData<float> &data)
{
  auto &seResult = data._stateEstimator->getResult();
  ...
  for (int r = 0; r < 8; r++)
  {
    if (A_pls[r] > b_pl[r])
    {
      if (changesupport && !get_shirt_iter)
      {
        first_run = true;
      }
      changesupport = false;
...
void ConvexMPCLocomotion::changeSupportIfNeeded_trotting(ControlFSMData<float> &data)
{
  auto &seResult = data._stateEstimator->getResult();
...
void ConvexMPCLocomotion::proposedCoMAndFeetPlan(ControlFSMData<float> &data)
{
  ...
  if (!changesupport)
  {
    ...
    nstep = ceil((x_qp - p_com).norm() / (0.075 * v_com1.norm()));
    // nstep = 3;
    int iteration = 0;
    int itemax = 5;
    float verror = 10.0f;
    ...
    while (iteration < itemax && fabs(verror) > 2)
    {
      for (int i = 0; i < 4; i++)
      {
        ...
        {
          footstepopt[ii].solve_qp(x0[i], x_des[i], nstep + ii - 1, seResult.position, data._quadruped->getHipLocation(i) + hipLength, 0.5 * v_com1, R, x_next, iteration, nrest);
        ...
        pos_foot_array[i] = pos_foot - seResult.rBody.transpose() * (data._quadruped->getHipLocation(i) + hipLength) - Vec3<float>(x_next(0), x_next(2), seResult.position(2));
        ...
        if (i == 0)
        {
          for (int iii = 1; iii <= nopt[i]; iii++)
          {
            Vec3<float> offsetx;
            offsetx.setIdentity();
            pos_foot_pred[i].block(2 * iii, 0, 1, 1) = pos_foot_pred[i].block(2 * iii, 0, 1, 1) + 0.03 * offsetx.head(1);
          }
        }
        else if (i == 2)
        {
...
void ConvexMPCLocomotion::run(ControlFSMData<float> &data)
{
  ...
  changeSupportIfNeeded_trotting(data);
  // changeSupportIfNeeded_bounding(data);

  if (!changesupport)
  {
    computex_des_trotting(data);
    // computex_des_bounding(data);
  }
  ...
  v_des_robot[0] = 0.025; //trotting
  // v_des_robot[1] = 0.00; //trotting

  // v_des_robot[0] = 0.0; //pacing
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    Vec3<float> offset(0, side_sign[i] * .092, 0);

    if (i < 2)
      offset[0] = 0.04; //vor-Beine soll nach vorner legen trotting
    else
      offset[0] = -0.02;

    // if(i<2)
    //      offset[0] = 0.04; // bounding
    //  else
    //      offset[0] = -0.04;
    ...
    float p_rel_max = 0.35f; //ysp
    // float p_rel_max = 0.3f; // zejun
  ...
  // Timer SQPTimer;
  if (first_run || (!changesupport && (iterationCounter - shirt_iter) % 14 == 0))
  // if (iterationCounter % 14 == 0)
  {
    proposedCoMAndFeetPlan(data);
    if (first_run && !get_shirt_iter)
    {
      first_run = false;
      shirt_iter = iterationCounter;
      get_shirt_iter = true;
    }
  }
  // proposedCoMAndFeetPlan(data);
  ...
  contact_state = gait->getContactState();
  printf("changesupport %d\n", changesupport);
  changesupport = true; //zejun
  // END of WBC Update

  // csvoutN(csvLog, seResult.vWorld, 3, firstRun);
  // csvoutN(csvLog, seResult.omegaWorld, 3, firstRun);
  ...
  // csvoutN(csvLog, pos_foot_array[0], 3, firstRun);
  // csvoutN(csvLog, pos_foot_array[1], 3, firstRun);
  // csvoutN(csvLog, pos_foot_array[2], 3, firstRun);
  // csvoutN(csvLog, pos_foot_array[3], 3, firstRun);
  // csvLog << nstep;

  // csvoutN(csvLog, data._legController->datas[0].tauEstimate, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[1].tauEstimate, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[2].tauEstimate, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[3].tauEstimate, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[0].tauActuatual, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[1].tauActuatual, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[2].tauActuatual, 3, firstRun);
  // csvoutN(csvLog, data._legController->datas[3].tauActuatual, 3, firstRun);
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data)
{
  ...
  float Q[12] = {1.25, 1.25, 10, 2, 2, 50, 0, 0, 0.3, 1.5, 1.5, 0.2}; // ysp

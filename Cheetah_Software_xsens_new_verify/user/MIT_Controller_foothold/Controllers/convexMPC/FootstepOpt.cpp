...
void FootstepOpt<T>::ss2qp(s16 horizon, Vec2<T> x_des, Vec3<T> p_com, Vec3<T> p_hip, Vec2<T> v_com, Mat3<T> R, DMat<T> x_next, s16 iteration, s16 nrest)
{
    ...
    for (int r = 0; r < horizon; r++)
    {
        ...
        if (iteration == 0)
        {
            ...
            x_com_pred = p_com.block(0, 0, 2, 1) + sqrt(0.3 / 9.81) * v_com + r * v_com * gait_cycle;
        ...
        else
        {
            ...
            else
            {
                ...
                // }
                int par = r;
                x_com_pred[0] = x_next(4 * (nrest + 5 * par), 0);
                x_com_pred[1] = x_next(4 * (nrest + 5 * par) + 2, 0);
                if (horizon < 2)
                {
                    x_com_pred[0] = x_next(4 * (4 + 4 * par), 0);
                    x_com_pred[1] = x_next(4 * (4 + 4 * par) + 2, 0);
                }
                else
                {
                    x_com_pred[0] = x_next(4 * (nrest + 5 * par), 0);
        ...
        slb.block(2 * r, 0, 2, 1) = x_com_pred + pow(1.0, double(r)) * templb;
        sub.block(2 * r, 0, 2, 1) = x_com_pred + pow(1.0, double(r)) * tempub;

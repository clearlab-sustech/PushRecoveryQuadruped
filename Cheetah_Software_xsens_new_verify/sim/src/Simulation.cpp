...
#include <unistd.h>
#include <fstream>
std::string result_file = std::string(THIS_COM) + std::string(RESULT_FILE);
    ...
    : _simParams(params), _userParams(userParams), _tau(12)
{
  ...
  if (!_robotParams.isFullyInitialized())
  {
    ...
    throw std::runtime_error("not all pa  rameters initialized from ini file");
  }
  // init IMU simulator
  ...
  if (readMatFromFile(result_file.c_str(), results, "results"))
  {
    size_t _size = results.size();
    if (size_t((GRID_X_MAX - GRID_X_MIN) / GRID_PRECISION) * size_t((GRID_Y_MAX - GRID_Y_MIN) / GRID_PRECISION) <= _size)
    {
    ...
    {
      size_t x_index = _size % size_t((GRID_X_MAX - GRID_X_MIN) / GRID_PRECISION);
      size_t y_index = _size / size_t((GRID_X_MAX - GRID_X_MIN) / GRID_PRECISION);
      vx = double(x_index) * GRID_PRECISION + GRID_X_MIN;
      vy = double(y_index) * GRID_PRECISION + GRID_Y_MIN;
      printf("read mat file successful...\n");
      printf("_size=%ld, x_index=%ld, y_index=%ld, vx = %f, vy = %f\n", _size, x_index, y_index, vx, vy);
    }
  }
  else
  {
    vx = GRID_X_MIN;
    vy = GRID_Y_MIN;
    printf("no mat file loaded...\n");
    printf("vx = %f, vy = %f\n", vx, vy);
  }
  csvLog.open("imuVSreal.txt");
}
                      ...
                      double dtHighLevelControl)
{
  ...
  // std::cout << "interval: " << timer.getSeconds() - s << std::endl;

  // bool contactStateFlag = false;
  // contactStateFlag = checkContact();
  // const FBModelState<double> &state = _simulator->getState();

  if (_currentSimTime >= _timeOfNextPushRecovery)
  {
    testForPushRecovery();
    _timeOfNextPushRecovery = _currentSimTime + TIMEFORRECOVERY;
    // printf("push time: %f\n", _currentSimTime);
    // getchar();
  }
  ...
  if (!start_test)
  {
    for (int i = 0; i < 8; i++)
    {
      if (_simulator->getContactForce(i).norm() > 1e-6)
      {
        contactViolation++;
        // printf("force %d: %f\n", i, _simulator->getContactForce(i).norm());
      }
    }
  }
  /* if (_currentSimTime >= _timeOfNextPushRecovery - TIMEFORRECOVERY && !start_test)
  {
    getchar();
  } */
}
...
void Simulation::highLevelControl()
{
  ...
  if (!start_test)
  {
    if (abs(vx) > 0.1)
    {
      _sharedMemory().simToRobot.cheaterState.acceleration(0) *= 0.5;
      _sharedMemory().simToRobot.vectorNav.accelerometer(0) *= 0.5;
    }
    if (abs(vy) > 0.1)
    {
      _sharedMemory().simToRobot.cheaterState.acceleration(1) *= 0.5;
      _sharedMemory().simToRobot.vectorNav.accelerometer(1) *= 0.5;
    }
    vx = 0;
    vy = 0;
  }
...
bool Simulation::checkContact()
{
  Vec4<double> contactForce, flag;
  for (size_t leg = 0; leg < 4; leg++)
  {
    size_t gcID = _simulator->getModel()._footIndicesGC.at(leg);
    contactForce(leg) = _simulator->getContactForce(gcID)[2];
    if (contactForce(leg) > 0)
    {
      flag(leg) = leg + 1;
    }
    else
  ...
  // int sum_flag = int(flag.sum());
  std::cout << "contactForce: " << contactForce.transpose() << std::endl;
  std::cout << "flag: " << flag.transpose() << std::endl;
  return true;
  /* Vec4<double> contactForce, flag;
  for (size_t leg = 0; leg < 4; leg++)
  {
  ...
  int sum_flag = int(flag.sum());
  std::cout << "sum_flag: " << sum_flag << std::endl;
  switch (sum_flag)
  {
  case 0:
  ...
  case 1:
    return ContactState::FR;
    break;

  case 2:
    return ContactState::FL;
    break;

  case 3:
    if (contactForce(0) > 1e-6 && contactForce(1) > 1e-6 && contactForce(2) < 1e-6)
    {
      return ContactState::FLFR;
    }
    else if (contactForce(0) < 1e-6 && contactForce(1) < 1e-6 && contactForce(2) > 1e-6)
    {
      return ContactState::HR;
    }
    else
    {
      throw std::runtime_error("contactState check error");
    }
    return ContactState::HR;
    break;

  case 4:
    if (contactForce(0) > 1e-6 && contactForce(2) > 1e-6 && contactForce(3) < 1e-6)
    {
      return ContactState::FRHR;
    }
    else if (contactForce(0) < 1e-6 && contactForce(2) < 1e-6 && contactForce(3) > 1e-6)
    {
      return ContactState::HL;
    }
    else
    {
      throw std::runtime_error("contactState check error");
  ...
  case 5:
    if (contactForce(0) > 1e-6 && contactForce(3) > 1e-6 && contactForce(1) < 1e-6 && contactForce(2) < 1e-6)
    {
      return ContactState::FRHL;
    }
    else if (contactForce(1) > 1e-6 && contactForce(2) > 1e-6 && contactForce(0) < 1e-6 && contactForce(3) < 1e-6)
    {
      return ContactState::FRHL;
    }
    else
  ...
  case 6:
    return ContactState::FLHL;
    break;

  case 7:
    return ContactState::HLHR;
    break;
  default:
    return ContactState::MORETHANTHREE;
    break;
  } */
}

void Simulation::testForPushRecovery()
{
  FBModelState<double> state = _simulator->getState();
  if (start_test)
  {
    ...
    kickVelocity << 0.0, 0.0, 0.0, vx, vy, 0.0;
    // kickVelocity.setZero();
    state.bodyVelocity += kickVelocity;
    _simulator->setState(state);
    start_test = false;

    MatrixXd results_new(results.size() + 1, 1);
    if (results.size() > 0)
    {
      results_new.topLeftCorner(results_new.size() - 1, 1) = results;
    }
    results_new(results_new.size() - 1, 0) = -1.0;
    savemat(results_new, result_file, "results");
  }
  else
  {
    if (state.bodyVelocity.bottomLeftCorner(3, 1).norm() < 0.8 && abs(state.bodyPosition[2] - 0.29) < 0.08 && contactViolation == 0 &&
        (state.q[2] > KNEE_JOINT_LIMIT_L && state.q[5] > KNEE_JOINT_LIMIT_L && state.q[8] > KNEE_JOINT_LIMIT_L && state.q[11] > KNEE_JOINT_LIMIT_L) &&
        (state.q[2] < KNEE_JOINT_LIMIT_U && state.q[5] < KNEE_JOINT_LIMIT_U && state.q[8] < KNEE_JOINT_LIMIT_U && state.q[11] < KNEE_JOINT_LIMIT_U) &&
        (abs(state.q[0]) < ABAD_JOINT_LIMIT && abs(state.q[3]) < ABAD_JOINT_LIMIT && abs(state.q[6]) < ABAD_JOINT_LIMIT && abs(state.q[9]) < ABAD_JOINT_LIMIT))
    {
      MatrixXd results_new(results.size() + 1, 1);
      if (results.size() > 0)
      {
      ...
      results_new(results_new.size() - 1, 0) = 1.0;
      savemat(results_new, result_file, "results");
      printf("[%.8f] successful test: vx = %f, vy = %f\n", _currentSimTime, vx, vy);
    }
    else
    {
      MatrixXd results_new(results.size() + 1, 1);
      ...
      results_new(results_new.size() - 1, 0) = -1.0;
      savemat(results_new, result_file, "results");
      printf("[%.8f] failed test: vx = %f, vy = %f\n", _currentSimTime, vx, vy);
    }
    std::cout << "vel norm: " << state.bodyVelocity.bottomLeftCorner(3, 1).norm() << std::endl
              << "pos: " << abs(state.bodyPosition[2] - 0.29) << std::endl
              << "contactViolation: " << contactViolation << std::endl;
    stop();
    handleControlError();
...
void Simulation::runAtSpeed(std::function<void(std::string)> errorCallback, bool graphics)
{
  ...
  while (_running)
  {
    ...
    if (!_window->IsPaused() && steps < desiredSteps)
    {
      ...
      // float s = freeRunTimer.getSeconds();
      step(dt, dtLowLevelControl, dtHighLevelControl);
      // std::cout << "dt = " << dt << ", duration: " << freeRunTimer.getSeconds() - s << std::endl;
      _simParams.unlockMutex();
      steps++;
    }
    else
